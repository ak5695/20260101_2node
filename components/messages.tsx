...
function PureMessages({
  ...
  return (
    <div className="relative flex-1">
      ...
      >
        <div className="mx-auto flex min-w-0 max-w-4xl flex-col gap-1 px-2 py-4 md:gap-2 md:px-2">
          {messages.length === 0 && <Greeting />}

          {messages.map((message, index) => {
             // Find previous user message if current is assistant
             let relatedUserQuestion = '';
             if (message.role === 'assistant' && index > 0) {
                 const prev = messages[index - 1];
                 if (prev.role === 'user') {
                     relatedUserQuestion = prev.parts.filter(p => p.type === 'text').map(p => p.text).join('') || '';
                 }
             }
